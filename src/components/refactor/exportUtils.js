...
## Ad Information
- **Platform:** ${adPlatform === 'ixr' ? 'IXR Interact' : adPlatform === 'focus' ? 'Focus' : adPlatform}
- **Template Type:** ${adMeta.templateType?.toUpperCase() || 'Unknown'}
...
${buildDeviceSpecs(adPlatform, width, height)}
...
function buildDeviceSpecs(adPlatform, width, height) {
  if (adPlatform === 'ixr') {
    return buildIXRDeviceSpecs(width, height)
  } else if (adPlatform === 'focus') {
    // TODO: Focus device specs to be added by focus dev
...
function buildIXRDeviceSpecs(width, height) {
  ...
  s += '    //External Link\n'
  s += '    function openExternalLinkFull(e, clickTag) {\n'
  s += '        if (typeof appHost !== \'undefined\') {\n'
  s += '            appHost.openExternalLinkFull(clickTag);\n'
  s += '        } else {\n'
  s += '            window.open(clickTag);\n'
  s += '        }\n'
  s += '    }\n\n'
  s += '    //External PDF\n'
...
function buildChecklist(features, animAnalysis, adMeta, allAssets, config) {
  ...
  if (animAnalysis?.hasAnimations && !animAnalysis.isUIOnly) {
    ...
    items.push('Confirm animation is wrapped in `onWallboardIdleSlideDisplay()` for CP ads (1080x1733)')
  }
